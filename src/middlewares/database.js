...
export class Database {
  ...
  update(table, id, data) {
    const rowNumber = this.#database[table].findIndex((row) => {
      return row.id == id;
    });

    if (rowNumber > -1) {
      this.#database[table][rowNumber].title = data.title
        ? data.title
        : this.#database[table][rowNumber].title;
      this.#database[table][rowNumber].description = data.description
        ? data.description
        : this.#database[table][rowNumber].description;
